...
def test_latest_returns_fixture_json(client, auth_headers, data_dir):
    ...
    r = client.get("/api/app-update/latest?track=release&component=phone", headers=auth_headers)
    assert r.status_code == 200
    body = r.json()
    assert body["version_code"] == SAMPLE_LATEST["version_code"]
    assert body["component"] == "phone"
    assert body["track"] == "release"
...
def test_unknown_component_returns_422(client, auth_headers):
    r = client.get("/api/app-update/latest?track=release&component=tablet", headers=auth_headers)
    assert r.status_code == 422
