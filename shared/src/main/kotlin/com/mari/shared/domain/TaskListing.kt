...
object TaskListing {
    ...
    fun sort(tasks: List<Task>, mode: SortMode): List<Task> = when (mode) {
        ...
        SortMode.A_Z -> tasks.sortedBy { it.description.lowercase() }
        SortMode.Z_A -> tasks.sortedByDescending { it.description.lowercase() }
    ...
    fun filter(
        ...
        return tasks.filter { task ->
            ...
            if (q.isNotEmpty() && !task.description.lowercase().contains(q)) return@filter false
            true
        }
    }
