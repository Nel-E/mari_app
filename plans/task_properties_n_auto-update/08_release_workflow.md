...
One codebase, two git branches, one backend, one channel with two tracks (`release` + `debug`). This document defines **how to cut a release** and **how to publish** so that phones and watches pick it up correctly.

## Branch Model

| Branch | Track published to | `versionName` suffix | Purpose |
|---|---|---|---|
| `debug` | `debug` | `-debug` | Active development; testable builds. |
| `release` | `release` | (none) | User-visible stable builds. |
...
1. Keep `versionName` human-readable; debug builds may append `-debug`.
...
4. Release and debug must **not** share a `versionCode` if one should upgrade the other.

| `versionName` | Padded | `versionCode` |
|---|---|---|
| `1.0.1.0-beta` | `1,0,1,0,0,1` | `101001` |
| `1.0.1.4-debug` | `1,0,1,4,0,1` | `101401` |
...
3. Publish phone:
   ...
   scripts/publish_phone.sh \
     ...
     --track debug \
     --version-code 101401 \
     --version-name 1.0.1.4-debug \
     --notification-title "Mari 1.0.1.4-debug available" \
     --notification-text "Deadline reminders and daily nudge."
   ```
4. Publish watch (same shape, `--track debug`).
5. On-device verification:
   - Phone running `1.0.1.3-debug` with beta track enabled picks up `1.0.1.4-debug` within one check cycle.
   - Watch picks up its update when triggered.
6. When stable: merge `debug` → `release`, drop the `-beta` suffix in `versionName`, bump the publish-sequence portion of `versionCode`, build release APKs, publish with `--track release`, commit: `chore(release): publish stable <versionName>`.
...
- [ ] Phone on debug track picks up the update.
- [ ] Watch on debug track picks up the update.
...
- [ ] `publish_phone.sh --track release` green.
- [ ] `publish_watch.sh --track release` green.
- [ ] Phone on release track picks up the update.
- [ ] Watch on release track picks up the update.
...
| User on debug never receives a later release build | Keep `versionCode` globally monotonic across both tracks; release publish must be higher than any beta build that may already be installed. |
...
- [ ] First debug release published from `debug` branch and installed on a real phone + watch from the RPi API.
- [ ] First release build published from `release` branch and installed.
...
- [ ] `not implemented` `release` branch created from `debug` (fast-forward on first stable cut)
- [ ] `not implemented` `versionCode` derivation documented and applied in `app/build.gradle.kts` and `wear/build.gradle.kts`
- [ ] `not implemented` First debug release published (phone + watch APKs) and installed on device
- [ ] `not implemented` First release build published and installed on device
...
- Debug and release builds of the same marketing version do **not** share a `versionCode`; publish order must remain globally monotonic across both tracks.
