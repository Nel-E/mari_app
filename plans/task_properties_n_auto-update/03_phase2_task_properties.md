...
- `app/src/main/kotlin/com/mari/app/ui/screens/add/AddTaskScreen.kt` — add deadline / color / reminder rows.
...
- `app/src/main/kotlin/com/mari/app/ui/screens/tasks/AllTasksScreen.kt` — render `colorHex` as row accent.
- `app/src/main/kotlin/com/mari/app/reminders/BootReceiver.kt` — re-arm deadline reminders on boot.
- `shared/src/main/kotlin/com/mari/shared/domain/ExecutionRules.kt` — accept deadline fields when creating a task.
...
Layout additions (below the description field):
...
Extend `BootReceiver.onReceive` to also re-arm `DeadlineReminderScheduler.schedule(...)` for each non-completed task with future `dueAt`.
...
4. `AllTasksScreenRenderTest` (Compose test or screenshot):
   - Row with `colorHex = "#FF8A65"` has the strip painted.
   - Overdue task chip is red.
5. Migration regression: `SchemaMigrationsV1ToV2Test` must still pass.
...
- [ ] `not implemented` `AddTaskScreen` / `AddTaskViewModel` wired with deadline + color + reminders
...
- [ ] `not implemented` `BootReceiver` re-arms deadline reminders on reboot
...
- On device reboot, `BootReceiver` re-arms all deadline alarms for non-completed tasks with a future `dueAt`; no alarm is lost after reboot.
